...
//   const operatorIds = ['42'];
  const lineIds = [];
  const stopIds = [];
  const startDate = '2024-09-09T04:00:00';
//   const endDate = '2024-09-30T03:59:59';
  ...
  const validationsBetweenDates = {
   ...
   //  'transaction.lineLongID': { $in: lineIds },
    'transaction.patternLongID': '2206_0_2',
  ...
  const validationsStream = await PCGIDB.ValidationEntity.find(validationsBetweenDates, { allowDiskUse: true, maxTimeMS: 999999999 }).stream();
  ...
  for await (const doc of validationsStream) {
    ...
    fs.appendFileSync(`dump_tx_42-set.csv`, isFirstDoc ? csvData : newLineCharacter + csvData);
